...
class Tty:
    def __init__(self, stdscr:window):
        ...
        self.center:list = [self.size[0] // 2, self.size[1] // 2]
...
class Unread:
    ...
    def get_ids(self):
        ...
        self.command = 'unseen'
        try:
            self.mail.response = self.mail.server.search(None, self.command)
...
class User:
    def __init__(self, tty, mail, unread, inbox):
        self.tty = tty
        self.mail = mail
        self.unread = unread
        self.inbox = inbox
        ...
        self.cfg_path:str = path.join(self.home, self.cfg_dir)  # pyright: ignore[reportCallIssue, reportArgumentType]
        ...
        try:
            ...
            try:
                with open(f'{self.cfg_email}', 'w') as mail:
                    mail.write(self.emailenc.decode())
            except OSError as e:
                self.tty.intro_print(f'{e.strerror}', 1)
                return False
            else:
    ...
    def login(self) -> None:
        response:tuple = self.mail.server.login(self.email, self.password)
        if response[0] == 'OK':
